...
uint2 = ArgumentDescriptor(
    name="uint2", n=2, reader=read_uint2, doc="Two-byte unsigned integer, little-endian."
)
...
uint4 = ArgumentDescriptor(
    name="uint4", n=4, reader=read_uint4, doc="Four-byte unsigned integer, little-endian."
)
...
def dis(pickle, out=None, memo=None, indentlevel=4, annotate=0):
    ...
    for opcode, arg, pos in genops(pickle):
        ...
        if len(stack) < numtopop:
            raise ValueError(
                "tries to pop %d items from stack with " "only %d items" % (numtopop, len(stack))
            )
        if numtopop:
            del stack[-numtopop:]
